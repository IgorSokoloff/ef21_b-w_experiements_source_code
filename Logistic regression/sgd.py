...
def sgd_estimator(A, x, b, la, g_ar, n_workers, tau_ar):
    sgrads = compute_stoch_grads (A, x, b, la, n_workers, tau_ar) #change compute sgrads
    assert(sgrads.shape==(n_workers,x.shape[0]))
    size_value_sent = 32
    return sgrads, size_value_sent

def sgd(x_0, A, b, A_0, b_0, stepsize, tau_ar, eps,la, n_workers, m_ar, Nsteps=100000):
    ...
    while stopping_criterion(sq_norm_ar[-1], eps, it, Nsteps):
        x = x - stepsize*g
        g_ar, size_value_sent = sgd_estimator(A,x, b, la, g_ar, n_workers, tau_ar)
        g = np.mean(g_ar, axis=0)
        it += 1
        if it%PRINT_EVERY ==0:
        ...
        if it%COMPUTE_GD_EVERY ==0:
            ...
            it_bits_ar.append(it*dim*size_value_sent)
...
step_size_sgd = (1/(L_0))*factor
tau_ar = np.array(n*prb).astype(dtype=int)

experiment_name = "sgd_nw-{0}_{1}x_{2}".format(n_w, myrepr(factor), myrepr(prb))

results = sgd(x_0, A, b, A_0, b_0, step_size_sgd, tau_ar, eps, la, n_w,n, Nsteps=nsteps)
print (experiment_name + f" finished in {results[0].shape[0]} iterations")
its_bits_sgd = results[0]
its_epochs_sgd = results[1]    # number of epochs
its_comm_sgd = results[2]      # communication complexity
norms_sgd = results[3]
sol_sgd = results[4]

save_data(its_bits_sgd, its_epochs_sgd, its_comm_sgd, norms_sgd, sol_sgd, experiment_name, project_path, dataset)
