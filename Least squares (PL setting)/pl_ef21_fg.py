...
def pl_ef21_fg_estimator(A, x, b, la, k, g_ar, n_workers):
...
def pl_ef21_fg(x_0, x_star, f_star, A, b, A_0, b_0, stepsize, eps,la,k, n_workers,experiment_name, project_path,dataset, Nsteps=100000):
    print(f"biased_diana_gd_nw-{n_workers}_k-{k}")
    dim = x_0.shape[0]
    ##
    #x_star = np.linalg.inv (A_0.T @ A_0 + la*n_workers * np.eye(dim)) @A.T @ b_0
    #f_star = least_squares_loss(x_star, A_0, b_0, la)
    ...
    while stopping_criterion(func_diff_ar[-1], eps, it, Nsteps):
        x = x - stepsize*g 
        g_ar, size_value_sent, grad = pl_ef21_fg_estimator(A, x, b, la, k, g_ar, n_workers)
        g = np.mean(g_ar, axis=0)
        ##
...
step_size_pl_ef21_fg = min(left_part, right_part) *factor

experiment_name = "pl-ef21-fg_nw-{0}_{1}x".format(n_w, myrepr(factor))
## }

begin_time = datetime.datetime.now()

results = pl_ef21_fg(x_0, x_star, f_star, X, y, X_0, y_0, step_size_pl_ef21_fg, eps,la, k_tk, n_w,experiment_name, project_path,dataset, Nsteps=nsteps)
