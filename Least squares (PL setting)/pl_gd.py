...
def gd(x_0,x_star, f_star, A, b, A_0, b_0, stepsize, eps,la,n_workers, Nsteps=100000):
    dim = x_0.shape[0]
    ...
    while stopping_criterion(func_diff_ar[-1], eps, it, Nsteps):
        ...
        it += 1
        its_bits_od_ar.append(it*dim*size_value_sent)
        its_bits_bd_ar.append(it*(2*dim)*size_value_sent)
...
def save_data(its_bits_od, its_bits_bd, its_epochs, its_comm, its_arg_res, func_diff, f_grad_norms, x_solution, experiment_name, project_path, dataset):
...
parser.add_argument('--tol', action='store', dest='tol', type=float, default=1e-4, help='tolerance')
...
mu_0 = np.min(np.linalg.eigvals(hess_f_0))
...
experiment_name = "pl-fg_nw-{0}_{1}x".format(n_w, myrepr(factor))

begin_time = datetime.datetime.now()

results = gd(x_0, x_star, f_star,X, y, X_0, y_0, step_size_gd, eps,la, n_w, Nsteps=nsteps)
...
print (experiment_name + f" finished in {its_comm.shape[0]} iterations; running time: {datetime.datetime.now() - begin_time}")

save_data(its_bits_od, its_bits_bd, its_epochs, its_comm, its_arg_res, func_diff, norms, sol, experiment_name, project_path,dataset)