...
def page_estimator(A, x,x_prev, b, la, v_ar, n_workers, prob, tau_ar, m_ar, tau_sum, m_total):
    ...
    return v_ar_new, num_oracle_epochs, c_k #np.mean(grads, axis=0)

def ef21_page(x_0, A, b, A_0, b_0, stepsize,prob, tau_ar, eps,la,n_workers, m_ar, Nsteps=100000):
...
step_size_ef21_page = (1/(L_0))*factor
tau_ar = np.array(n*prb).astype(dtype=int)

experiment_name = "page_nw-{0}_{1}x_{2}".format(n_w, myrepr(factor), myrepr(prb))

prob = np.mean(tau_ar/(tau_ar + n))

results = ef21_page(x_0, A, b, A_0, b_0, step_size_ef21_page, prob, tau_ar, eps, la, n_w, n, Nsteps=nsteps)
...
save_data(its_bits_ef21_page, its_epochs_ef21_page, its_comm_ef21_page, norms_ef21_page, sol_ef21_page, experiment_name, project_path, dataset)
